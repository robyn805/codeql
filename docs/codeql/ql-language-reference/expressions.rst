...
when ``getPrice(f)`` has multiple results.
...
*monotonic* semantics for ``sum``, Alice always buys orange and one
apple, and one line of output is produced for each *way* she can
...
``sum`` output at all, because there *is no way* for Charles to buy
one apple plus one banana.
...
In actual QL usage, it is quite rare to use monotonic aggregates with
the *goal* of having multiple output, as in the "Alice" case of this
example. The more significant point is the "Charles" case: As long as
there's no price for bananas, no output is produced for him. This
means that if we later do learn of a banana price, we don't need to
*remove* any output tuple already produced. The importance of this is
that the monotonic aggregate semantics works well with a
fixpoint-based definition for recursion, so it will be meaningul to
let the ``getPrice`` predicate be mutually recursive with the count
